...
interface AsciiEffectProps {
  ...
  invert?: boolean
  color?: boolean
  postfx?: Record<string, number | boolean | string>
  resolution?: Vector2
  mousePos?: Vector2
}
...
export const AsciiEffect = forwardRef<Effect, AsciiEffectProps>((props, ref) => {
  ...
  // Effect is created once; updates happen via module-level variables in update()
  ...
  const effect = useMemo(
    () =>
      new AsciiEffectImpl({
        ...
        color,
        style: styleNum,
        postfx: postfx as Record<string, number | boolean>,
        resolution,
        mousePos,
      }),
    []
